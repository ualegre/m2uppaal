...
public class NTA extends UppaalElement {
	...
	for (Automaton template : automata) {
		...
		if (transition.getSelect() != null) {
		    ...
		    // offset must be added with spaceBetweenLabels as many
		    // times there are newlines
	...
	for (Automaton automaton : automata) {
		...
		if (transition.getSource() == transition.getTarget()) {
			...
			if (location.equals(transition.getSource())) {
			    transition.addNail(new Nail(location.getPosX() + nailX1, location.getPosY() + nailY1));
			    transition.addNail(new Nail(location.getPosX() + nailX2, location.getPosY() + nailY2));
			}
		    }
		}
	    }
    ...
    @Deprecated
    private void updateLocations(Automaton automaton, Location currentLocation) {
	...
	for (Transition transition : automaton.getTransitions()) {
		...
		if (transition.getSource().equals(currentLocation)) {
			...
			if (location.equals(transition.getTarget())) {
				...
				location.setPosX(currentLocation.getPosX() + xInterval * locationOffset);
				location.setPosY(currentLocation.getPosY() + yInterval);
				succ.add(location);
				locationOffset++;
				// Check if it is a A -> B -> A set of
				// transitions
				for (Transition trans2 : automaton.getTransitions()) {
				    if (trans2.getSource() == transition.getTarget()
					    && trans2.getTarget() == transition.getSource()) {
					if (trans2.getNails().size() == 0 && transition.getNails().size() == 0) {
					    ...
					    // from the X position
					    location.setPosX(
						    currentLocation.getPosX() + xInterval * (locationOffset) - 50);
					    location.setPosY(currentLocation.getPosY());
					    trans2.addNail(new Nail(currentLocation.getPosX() + nailX1,
						    currentLocation.getPosY() + nailY1));
					    transition.addNail(new Nail(currentLocation.getPosX() + nailX2,
						    currentLocation.getPosY() + nailY2));
					    break;
					}
				    }
				}
				...
				if (currentLocation.getPosX() == location.getPosX()
					&& currentLocation.getPosY() > location.getPosY()) {
				    currentLocation.setPosX(currentLocation.getPosX() + xInterval - 50);
				}
			    }
			}
		    }
	...
	return new Document(this.generateXMLElement(),
		new DocType("nta", "-//Uppaal Team//DTD Flat System 1.1//EN", "flat-1_1.dtd"));
    }
