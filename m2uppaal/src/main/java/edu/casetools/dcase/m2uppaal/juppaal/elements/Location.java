...
import edu.casetools.dcase.m2uppaal.juppaal.labels.Invariant;
...
public class Location extends PositionedUppaalElement {
    ...
    /**
     * Creates a new location with the given name and id The name and id must be
     * unique in the template, which is not checked here
     * 
     * @param name
     *            The name of the location
     * @param id
	...
	if (inv != null && !getExpRate().toString().equals(""))
	    result.addContent(getExpRate().generateXMLElement());
